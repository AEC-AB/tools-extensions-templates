...
applyTo: '**/*.cs'
---
# Navisworks Automation Extension Guide

This template is for extensions that execute inside Navisworks and need Navisworks model or selection APIs.

Use the local extension docs through the `extension-docs` MCP tool:

- `operation=index` to list available extension docs
- `operation=search` for focused topics such as `navisworks selection`, `autofill`, or `args validation`
- `operation=content` with document ids such as `quick-start`, `args-developer-guide`, `cookbook`, `reference`, or `navisworks`

Suggested reading order:

1. `quick-start` for the template shape
2. `navisworks` for Navisworks-specific behavior
...
2. `*Command.cs` runs inside Navisworks and works with the Navisworks API.
...
Use `INavisworksExtensionContext` in command execution. This context inherits `IExtensionContext`.
